...
Changed from run as to managed identity
.PRIVATEDATA 
#>
<# 
.DESCRIPTION 
...
$vmIds | ForEach-Object {
    ...
    $rg = $split[4];
    $name = $split[8];
    Write-Output ("Subscription Id: " + $subscriptionId +" rg:" +$rg +" vmname :"+$vmId)
    ...
    if($state -in $startableStates) {
		...
		$job=Start-AzVM -ResourceGroupName $rg -Name $name -DefaultProfile $context -AsJob
        #Store the VM we started so we remember to shut it down later
        $updatedMachines += $vmId
        #$newJob = Start-ThreadJob -ScriptBlock { param($resource, $name, $sub) $context = Select-AzSubscription -Subscription $sub; Start-AzVM -ResourceGroupName $resource -Name $name -DefaultProfile $context} -ArgumentList $rg,$name,$subscriptionId
        $jobIDs.Add($job.Id)
    }else {
