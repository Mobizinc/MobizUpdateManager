...
Changed run as to managed identity
.PRIVATEDATA 
#>
<# 
.DESCRIPTION 
...
$vmIds | ForEach-Object {
    ...
    if($state -in $stoppableStates) {
		...
		$vmContext = Select-AzSubscription -Subscription $subscriptionId		
        $newJob= Stop-AzVM -ResourceGroupName $rg -Name $name -Force -DefaultProfile $vmContext -AsJob 
